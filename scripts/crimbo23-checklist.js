...
function _defineProperty(obj, key, value) { key = property_toPropertyKey(key); if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
function property_toPropertyKey(t) { var i = property_toPrimitive(t, "string"); return "symbol" == typeof i ? i : String(i); }
function property_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }
...
function doMonster(monster, locketMonsters, hasLocket) {
  var locketed = locketMonsters.includes(monster);
  var factoidsLeft = 3 - (0,external_kolmafia_namespaceObject.monsterFactoidsAvailable)(monster, true);
  var ignoreItems = template_string_$items(main_templateObject || (main_templateObject = main_taggedTemplateLiteral(["Elf Guard MPC, Crimbuccaneer piece of 12, plum, orange, lemon, cherry, strawberry, pear, lemon, grapefruit, lime, raspberry, kumquat, peach, papaya, kiwi, blackberry, tangerine, bottle of gin, bottle of rum, bottle of vodka, bottle of tequila, bottle of whiskey, boxed wine"])));
  var drops = (0,external_kolmafia_namespaceObject.itemDropsArray)(monster).map(each => each.drop).filter(each => !ignoreItems.includes(each));
  var haveDrops = drops.filter(each => have(each));
  var needDrops = drops.filter(each => !have(each));
  if ((locketed || !hasLocket) && factoidsLeft === 0 && needDrops.length === 0) {
    (0,external_kolmafia_namespaceObject.print)("".concat(monster.name, " ... DONE"), 'green');
    return true;
  }
  (0,external_kolmafia_namespaceObject.print)();
  (0,external_kolmafia_namespaceObject.print)(monster.name, 'black');
  if (hasLocket) {
    if (locketed) (0,external_kolmafia_namespaceObject.print)('&#9745; in locket', 'black');else (0,external_kolmafia_namespaceObject.print)('&#9744; not in locket', 'red');
  }
  if (factoidsLeft === 0) {
    (0,external_kolmafia_namespaceObject.print)('&#9745; have all factoids', 'black');
  } else {
    (0,external_kolmafia_namespaceObject.print)("&#9744; ".concat(factoidsLeft, " factoids remaining"), 'red');
  }
  haveDrops.forEach(each => (0,external_kolmafia_namespaceObject.print)("&#9745; have ".concat(each.name), 'black'));
  needDrops.forEach(each => (0,external_kolmafia_namespaceObject.print)("&#9744; need ".concat(each.name), 'red'));
  return false;
}
function main() {
  (0,external_kolmafia_namespaceObject.print)('--------------------------------------');
  (0,external_kolmafia_namespaceObject.print)('Crimbo 23 Checklist');
  (0,external_kolmafia_namespaceObject.print)('--------------------------------------');
  if ((0,external_kolmafia_namespaceObject.getRevision)() < 27742) {
    (0,external_kolmafia_namespaceObject.print)('This script requires mafia version >= 27742. Please update mafia.', 'red');
    return;
  }
  var hasLocket = CombatLoversLocket_have();
  if (hasLocket && reminiscesLeft() === 0) {
    hasLocket = false;
    (0,external_kolmafia_namespaceObject.print)('No reminisces left, skipping locket.', 'red');
  }
  (0,external_kolmafia_namespaceObject.print)();
  var locations = $locations(main_templateObject2 || (main_templateObject2 = main_taggedTemplateLiteral(["Abuela's Cottage (Contested), The Embattled Factory, The Bar At War, A Cafe Divided, The Armory Up In Arms"])));
  var genericMonsters = $monsters(main_templateObject3 || (main_templateObject3 = main_taggedTemplateLiteral(["Crimbuccaneer military school dropout, Crimbuccaneer new recruit, Crimbuccaneer privateer, Elf Guard conscript, Elf Guard convict, Elf Guard private"])));
  var locketMonsters = hasLocket ? unlockedLocketMonsters() : [];
  (0,external_kolmafia_namespaceObject.print)('Generic Monsters', 'blue');
  genericMonsters.forEach(monster => doMonster(monster, locketMonsters, hasLocket));
  (0,external_kolmafia_namespaceObject.print)();
  (0,external_kolmafia_namespaceObject.print)();
  locations.forEach(location => {
    (0,external_kolmafia_namespaceObject.print)(location.toString(), 'blue');
    (0,external_kolmafia_namespaceObject.getMonsters)(location).filter(monster => !genericMonsters.includes(monster)).forEach(monster => doMonster(monster, locketMonsters, hasLocket));
    (0,external_kolmafia_namespaceObject.print)();
    (0,external_kolmafia_namespaceObject.print)();
  });
  (0,external_kolmafia_namespaceObject.print)('Skills', 'blue');
  (0,external_kolmafia_namespaceObject.print)();
  var skills = $skills(main_templateObject4 || (main_templateObject4 = main_taggedTemplateLiteral(["Fruit Recognition, Old-School Cocktailcrafting, Elf Guard Cooking, Elf Guard Relaxation Techniques, Elf Guard Extortion Techniques"])));
  skills.forEach(skill => {
    if ((0,external_kolmafia_namespaceObject.haveSkill)(skill)) (0,external_kolmafia_namespaceObject.print)("&#9745; ".concat(skill.name), 'green');else (0,external_kolmafia_namespaceObject.print)("&#9744; ".concat(skill.name), 'red');
  });
  (0,external_kolmafia_namespaceObject.print)();
}
